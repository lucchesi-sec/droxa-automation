...
export default function AnimatedPlatformsCard({ area }: AnimatedPlatformsCardProps) {
  const logos = [
    { component: <ClaudeLogo className="h-4 w-4" />, name: "Claude" },
    { component: <GoCopilot className="h-6 w-6 dark:text-white" />, name: "Copilot" },
    { component: <OpenAILogo className="h-8 w-8 dark:text-white" />, name: "OpenAI" },
    { component: <MetaIconOutline className="h-6 w-6" />, name: "Meta" },
    { component: <GeminiLogo className="h-4 w-4" />, name: "Gemini" },
  ];
  ...
  React.useEffect(() => {
    // Animate each sequence item individually
    sequence.forEach(([selector, keyframes, options]) => {
      // @ts-ignore
      animate(selector as string, keyframes, { 
        ...(options as object), 
        repeat: Infinity,
        repeatDelay: 1,
      });
        ...
        <div className="border-0.75 relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl p-6 md:p-6 dark:shadow-[0px_0px_27px_0px_#2D2D2D]">
          <div className="relative flex flex-1 flex-col justify-between gap-3">
            <div className="w-fit rounded-lg border border-gray-600 p-2">
              <IntegrationIcon className="h-4 w-4 text-black dark:text-white" />
            </div>
            <div className="space-y-3">
              ...
              <h2 className="font-sans text-sm/[1.125rem] text-black md:text-base/[1.375rem] dark:text-neutral-400">
                Seamless integration with leading AI and automation platforms
              </h2>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {logos.map((logo, index) => (
                  <div
                    key={index}
                    className={`h-8 w-8 rounded-full flex items-center justify-center bg-[rgba(248,248,248,0.01)] circle-${index + 1}`}
                  >
                    {logo.component}
                  </div>
                ))}
...
export const ClaudeLogo = ({ className }: { className?: string }) => {
  return (
    ...
    >
      <rect fill="#CC9B7A" width="512" height="512" rx="104.187" ry="105.042" stroke="none" />
      <path
        fill="#1F1F1E"
        fillRule="nonzero"
